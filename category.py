...
class categoryClass:
    ...
    def get_listbox_items(self):
        """Return all items in the Listbox as a list."""
        return [self.list_box.get(i) for i in range(self.list_box.size())]
    ...
    def remove_btn(self):
        """Remove the selected item from the Listbox and the database."""
    ...
    def add_fil(self):
        """Open a new window to add a new item to the Listbox and the database."""
        self.new_win = Toplevel(self.root)
        self.new_win.geometry("600x600+500+130")
        self.new_win.title("اضافة مجموعات")
        ...
        def save_item():
            ...
            if item_name:
                ...
                messagebox.showinfo("Success", "تم اضافة المجموعة بنجاح", parent=self.root)
    ...
    def search_items(self, event):
        """Search and filter items in the Listbox based on the search query."""
