...
class App:
    ...
    def toggle_storage_dropdown(self):
        if self.storage_dropdown_frame: 
            ...
            btn_category = Button(self.storage_dropdown_frame, text="المجموعة",command=self.new_cat_add, cursor="hand2", font=("times new roman", 12, "bold"), bg='white').pack(side=TOP, fill=X)
    def new_items_add(self):
        self.new_win = Toplevel(self.root)
        self.new_obj = add_items(self.new_win)
    
    def new_fill_add(self):
        self.new_win =Toplevel(self.root)
